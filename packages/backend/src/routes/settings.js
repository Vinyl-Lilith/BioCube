...
// User deletes their own account. Requires current password confirmation.
// Head admin cannot delete their own account (would lock everyone out).
router.delete('/account', async (req, res, next) => {
  try {
    ...
    // Head admin cannot self-delete — would orphan the system
    if (req.user.role === 'head_admin') {
      return res.status(403).json({ error: 'Head admin account cannot be self-deleted. Ask another head admin or use the admin panel.' });
    }

    // Re-fetch with password hash for comparison
    ...
    await require('../models/models').AdminRequest.deleteMany({ userId: req.user._id });

    await require('../models/models').ActivityLog.create({
      userId:   req.user._id,
      username,
